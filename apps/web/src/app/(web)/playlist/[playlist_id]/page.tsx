...
import { CreatePlaylistModal } from "../CreateUpdatePlaylist";
...
function PlaylistVideos({ playlistId }: { playlistId: string }) {
  ...
  async function getPlaylistVideos(page: number) {
    setIsLoading(true);
    try {
      const res = await trpcClient.playlist.getPlaylistVideo.query({
        playlist_id: playlistId,
        page: page,
        limit: 10,
      });
      setVideos(pre =>
        [...pre, ...res.video].filter(
      ...
      >
        ...
        <div className="flex justify-center items-center">
          ...
          {(isError || hasMore) && !isLoading && (
            <Button onClick={() => getPlaylistVideos(page)}>
...
export default function PlaylistPage({
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.playlist.getPlaylist.useQuery({ id: playlistId });
      ...
      <div className="w-1/3">
        <Card>
          <CardContent className="p-4">
            ...
            {user.id === details.creater_id && (

            <div className="flex gap-2">
              <Button onClick={() => setIsModalOpen(true)}>Edit Playlist</Button>
              <Button variant={"destructive"} onClick={() => {
                if (confirm("are you sure you want to delete this playlist?")) {
                  trpcClient.playlist.deletePlaylist.mutate({id: details.id})
