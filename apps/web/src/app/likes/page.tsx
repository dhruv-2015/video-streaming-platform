...
type LikeVideo = RouterOutputs["like"]["getMyLikesVideo"]["videos"][0];
...
export default function PlaylistPage({
  ...
  const [videos, setVideos] = useState<LikeVideo[]>([]);
  ...
  async function getLikeVideos(page: number) {
    setIsLoading(true);
    try {
      const res = await trpcClient.like.getMyLikesVideo.query({
        page: page,
        limit: 10,
      });
      setVideos(pre =>
  ...
  if (videos.length === 0) {
    return (
        <div className="flex justify-center items-center h-44">
            <p>you don't have any like videos</p>
        </div>
    )
  }
//   if (isError) {
